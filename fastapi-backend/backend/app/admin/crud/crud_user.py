...
class CRUDUser(CRUDPlus[User]):
    ...
    async def get_by_login_identifier(self, db: AsyncSession, identifier: str) -> User | None:
        """
        通过用户名或登录别名获取用户(仅用于登录入口)

        登录时接受 `username`(企微 userid)或 `login_alias`(kdit-昵称)两种标识,
        业务权限、审计日志等仍以原 `username` 为准。

        :param db: 数据库会话
        :param identifier: username 或 login_alias
        :return:
        """
        stmt = select(User).where(or_(User.username == identifier, User.login_alias == identifier))
    ...
    async def get_select(self, dept: int | None, username: str | None, phone: str | None, status: int | None) -> Select:
        ...
        :param dept: 部门 ID
        :param username: 搜索关键字,匹配 username / nickname / real_name / 角色名
        :param phone: 电话号码
        :param status: 用户状态
        :return:
        """
        ...
        if username:
            ...
            role_subq = (
                ...
                .where(Role.name.ilike(term))
            )
            whereclauses.append(
                or_(
                    User.username.ilike(term),
                    User.nickname.ilike(term),
                    User.real_name.ilike(term),
                    User.id.in_(role_subq),
                )
            )
